...
namespace DashboardBase::Components {
    class Window {
    ...
    public:
        ...
        explicit Window(std::string  window_title , const int& width, const int& height)
            : window_title_(std::move(window_title)), width_(width), height_(height) {
            init_window();
            init_renderer();
        };

        explicit Window(const Params& params): Window(params.window_title, params.width, params.height) {
